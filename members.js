...
const PERMISSION_ALIASES = {
  poll: ['canCreatePoll','canManagePolls'],
  announcement: ['canCreateAnnouncement','canManageAnnouncements'],
  recruitment: ['canManageRecruitment'],
  sport: ['canManageLineups','canManageMatches','canManageAttendance','canManageStatistics','canManageActivity'],
  system: ['canViewSystemHealth','canRunIntegrityCheck','canRunSystemRepair','canReconcileAccess','canRefreshCenters','canViewPlayerWatch','canViewAccessControl','canViewAudit','canRunEaSync']
};
...
function ryvexNotice(message,type='error'){
  let box=document.getElementById('ryvexNotice');
  if(!box){box=document.createElement('div');box.id='ryvexNotice';box.className='ryvex-notice';document.body.append(box);}
  box.textContent=String(message||'');box.dataset.type=type;box.classList.add('show');clearTimeout(ryvexNotice._t);ryvexNotice._t=setTimeout(()=>box.classList.remove('show'),type==='error'?5200:3200);
}
function ryvexConfirm(message,{title='Potvrdit akci',danger=false,confirmLabel='Potvrdit'}={}){
  return new Promise(resolve=>{
    let d=document.getElementById('ryvexConfirmDialog');
    if(!d){d=document.createElement('dialog');d.id='ryvexConfirmDialog';d.className='ryvex-dialog';d.innerHTML='<form method="dialog"><small>RYVEX SECURE ACTION</small><h3 id="ryvexConfirmTitle"></h3><p id="ryvexConfirmText"></p><div class="ryvex-dialog-actions"><button value="cancel" class="inline-action" type="submit">Zrušit</button><button value="ok" id="ryvexConfirmOk" class="inline-action" type="submit">Potvrdit</button></div></form>';document.body.append(d);}
    d.querySelector('#ryvexConfirmTitle').textContent=title;d.querySelector('#ryvexConfirmText').textContent=String(message||'');const ok=d.querySelector('#ryvexConfirmOk');ok.textContent=confirmLabel;ok.dataset.danger=danger?'1':'0';
    const close=()=>{d.removeEventListener('close',close);resolve(d.returnValue==='ok')};d.addEventListener('close',close);d.showModal();
  });
}
...
async function saveVote(pollId,choice){try{await post('/api/vote',{pollId,choice});await loadState(true)}catch(e){ryvexNotice(`Hlas se nepodařilo uložit: ${e.message}`)}}
...
$('chatForm')?.addEventListener('submit',async e=>{e.preventDefault();const input=$('chatInput'),content=input.value.trim();if(!content||!state.chatChannelId)return;input.disabled=true;try{await post('/api/chat-send',{channelId:state.chatChannelId,content});input.value='';await loadChatMessages()}catch(err){ryvexNotice(`Zprávu se nepodařilo odeslat: ${err.message}`)}finally{input.disabled=false;input.focus()}});
...
async function setAdminRequestStatus(requestId,status,button){
  if(button)button.disabled=true; try{await post('/api/admin-request-status',{requestId,status});await loadAdminRequests(true)}catch(err){ryvexNotice(`Stav požadavku se nepodařilo změnit: ${err.message}`)}finally{if(button)button.disabled=false}
}
function renderPlatformObservability(live){
  ...
  (center.rows||[]).forEach(row=>{const card=node('div',`platform-status-card ${row.status||'waiting'}`),dot=node('i','platform-status-dot'),copy=node('div');copy.append(node('b','',text(row.label,row.platform)),node('small','',`${labels[row.status]||String(row.status||'WAITING').toUpperCase()}${row.version?` • v${row.version}`:''}${row.lastSeenAt?` • ${fmt(row.lastSeenAt).time}`:''}`));card.append(dot,copy);grid.append(card)});
  const doctor=live?.centers?.admin?.connectionDoctor;
  if(doctor?.manager){const c=node('div',`platform-status-card ${doctor.manager.discordReady?'online':'degraded'}`),dot=node('i','platform-status-dot'),copy=node('div');copy.append(node('b','',`Manager ${text(doctor.manager.version,'')}`),node('small','',`${doctor.manager.discordReady?'DISCORD ONLINE':'DISCORD OFFLINE'} • EA ${text(doctor.ea?.status,'—').toUpperCase()} • polls ${doctor.voting?.openPolls??0}`));c.append(dot,copy);grid.append(c);}
  if(grid&&!grid.children.length)grid.append(node('div','empty-inline','Platform heartbeat zatím nebyl přijat.'));
...
function renderManagement(live){
  ...
  const p=live.permissions||{};
  const cap=(k)=>Boolean(p.isOwner||p[k]);
  const byId=(id,perm)=>{const el=$(id);if(el)el.hidden=!cap(perm);};
  byId('playerWatchRefresh','canViewPlayerWatch'); byId('systemEaSync','canRunEaSync'); byId('refreshDiscordCenters','canRefreshCenters');
  byId('systemRepair','canRunSystemRepair'); byId('systemIntegrity','canRunIntegrityCheck'); byId('accessReconcile','canReconcileAccess');
...
$('activePollAdminBody')?.addEventListener('click',async e=>{const b=e.target.closest('[data-close-poll]');if(!b)return;if(!await ryvexConfirm('Opravdu ukončit aktuální hlasování?',{title:'Ukončit hlasování'}))return;b.disabled=true;b.textContent='Ukončuji…';try{await post('/api/manage',{action:'close_poll',pollId:b.dataset.closePoll});await loadState(true)}catch(err){ryvexNotice(`Hlasování se nepodařilo ukončit: ${err.message}`)}finally{b.disabled=false}});
...
$('toggleWeeklyAuto')?.addEventListener('click',async e=>{const enabled=!Boolean(state.live?.centers?.voting?.weekly?.enabled);setBusy(e.currentTarget,true);try{await ops({action:'voting.auto.set',enabled});await loadState(true)}catch(err){ryvexNotice(`Auto tvorbu nelze změnit: ${err.message}`)}finally{setBusy(e.currentTarget,false)}});
$('toggleVotingCleanup')?.addEventListener('click',async e=>{const enabled=!Boolean(state.live?.centers?.voting?.cleanup?.enabled);setBusy(e.currentTarget,true);try{await ops({action:'voting.cleanup.set',enabled});await loadState(true)}catch(err){ryvexNotice(`Noční úklid nelze změnit: ${err.message}`)}finally{setBusy(e.currentTarget,false)}});
$('runVotingCleanup')?.addEventListener('click',async e=>{if(!await ryvexConfirm('Vyčistit kanál hlasování a zachovat řídicí centrum?',{title:'Noční úklid hlasování',danger:true,confirmLabel:'Vyčistit'}))return;setBusy(e.currentTarget,true,'Čistím…');try{await ops({action:'voting.cleanup.run'});await loadState(true)}catch(err){ryvexNotice(`Úklid selhal: ${err.message}`)}finally{setBusy(e.currentTarget,false)}});
$('openPollAdminList')?.addEventListener('click',async e=>{const b=e.target.closest('[data-cancel-poll]');if(!b)return;if(!await ryvexConfirm('Zrušit toto hlasování?',{title:'Zrušit hlasování',danger:true,confirmLabel:'Zrušit hlasování'}))return;setBusy(b,true,'Ruším…');try{await ops({action:'voting.cancel',pollId:b.dataset.cancelPoll});await loadState(true)}catch(err){ryvexNotice(`Hlasování nelze zrušit: ${err.message}`)}finally{setBusy(b,false)}});
...
$('lineupPublish')?.addEventListener('click',async e=>{if(!await ryvexConfirm('Publikovat aktuální sestavu a oznámit ji hráčům?',{title:'Publikovat sestavu',confirmLabel:'Publikovat'}))return;setBusy(e.currentTarget,true,'Publikuji…');try{await ops({action:'lineup.publish'});await loadState(true);setMessage('lineupAdminMessage','Sestava publikována.')}catch(err){setMessage('lineupAdminMessage',`Chyba: ${err.message}`)}finally{setBusy(e.currentTarget,false)}});
...
// RYVEX Website 2.6 — Manager 5.12 hardened controls
const systemOp=(id,action,ok)=>$(id)?.addEventListener('click',async e=>{setBusy(e.currentTarget,true);setMessage('systemControlMessage','Pracuji…');try{await ops({action,...(action==='system.integrity'?{repair:false}:{})});setMessage('systemControlMessage',ok);await loadState(true)}catch(err){setMessage('systemControlMessage',`Chyba: ${err.message}`)}finally{setBusy(e.currentTarget,false)}});
