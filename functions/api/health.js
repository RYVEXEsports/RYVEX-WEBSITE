export async function onRequestGet({env}){
  ...
  if(/^https:\/\//i.test(base)&&key){
    try{
      const r=await fetch(base+'/api/v1/platform-heartbeat',{headers:{'Accept':'application/json','X-RYVEX-API-KEY':key,'X-RYVEX-PLATFORM':'website','X-RYVEX-PLATFORM-VERSION':'2.5.1'},cf:{cacheEverything:false}});
  ...
  return Response.json({ok:true,service:'RYVEX Website',version:'2.5.1',manager},{headers:{'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}});
}
