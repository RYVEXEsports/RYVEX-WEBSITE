...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    if (!memberResponse.ok) {
      return redirect("/?auth=not_member#members", [clearStateCookie()]);
    }

    const now = Math.floor(Date.now() / 1000);
    const tokenValue = await signSession({
      sub: String(user.id),
      username: String(user.username || "RYVEX member").slice(0, 40),
      globalName: String(user.global_name || user.username || "RYVEX member").slice(0, 60),
    ...
    return redirect("/members.html", [sessionCookie(tokenValue), clearStateCookie()]);
  } catch {
